...
1. Uses the reference signal to estimate the unwanted noise.
2. Calculates the estimated noise component.
3. Computes the error signal between the primary signal and estimated noise.
4. Updates the filter coefficients using the LMS algorithm.
5. Uses the resulting error signal as the noise-reduced output.
...
* ADC signal normalization
* High-pass filtering
* Voice activity detection
* Adaptive LMS noise estimation
* Circular buffers for storing recent samples
* Real-time error-signal calculation

The **error signal** produced by the adaptive filter is used as the final noise-reduced audio output.
...
The objective of this project is to demonstrate how **adaptive signal processing and embedded systems** can be combined to implement a real-time noise cancellation system using an ESP32 and dual-microphone configuration.
